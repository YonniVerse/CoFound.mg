...
**État du workspace** : squelette API E-12 implémenté et testé ; rapports Markdown d’analyse non suivis conservés hors commit
...
La PR #34 (E-08 backend + UI-36) est fusionnée dans `dev`. La branche E-12 est issue de ce `dev` synchronisé. Les tests API E-12 passent avec 36 tests au total ; les rapports d’analyse locaux ne sont pas suivis par Git et ne doivent pas être inclus dans un commit métier.
...
Compléter E-12 avec les validations métier de profil, puis préparer la PR : vérifier la visibilité publique/privée, l’édition transactionnelle et la compatibilité avec E-13 (onboarding progressif).
