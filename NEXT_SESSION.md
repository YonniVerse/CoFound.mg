...
M-05 à M-08 sont intégrés dans `dev`. M-14 ajoute les contrats Zod du signalement, `ReportService`, `ReportController`, `ReportModule`, la route POST `/reports` pour les cibles PROFILE, MESSAGE, PROJECT et POST, ainsi qu’une création transactionnelle sans identité civile dans la réponse. Le socle API et ses tests sont en cours de revue ; les boutons sur les écrans métier et les tests HTTP M-14 restent à compléter.

## 6. Prochaine action

Faire relire et fusionner la PR #64 après vérification de ses secrets staging. Finaliser M-14 en ajoutant les boutons aux profils, messages, projets et publications, puis écrire les tests HTTP. Le workflow staging ne pourra s’exécuter qu’après configuration de `STAGING_*`, `GHCR_READ_TOKEN` et `GHCR_USERNAME` dans l’environnement GitHub `staging`. Après M-14, suivre `S-01 → S-02 → S-03/S-04`. M-12, M-13, M-15 et M-16 restent à traiter par leurs propriétaires.
