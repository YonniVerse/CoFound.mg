...
export class InstitutionAffiliationController {
  constructor(private readonly service: InstitutionAffiliationService) {}
  @Get('institution/affiliations') @RequirePermissions(Permission.ORG_MANAGE)
  ...
  @Patch('affiliations/:id') @RequirePermissions(Permission.ORG_MANAGE) @AuditAction('AFFILIATION_STATUS_UPDATE', 'Affiliation')
  ...
  @Post('institution/affiliations/bulk-status') @RequirePermissions(Permission.ORG_MANAGE) @AuditAction('AFFILIATION_BULK_STATUS', 'Affiliation')
