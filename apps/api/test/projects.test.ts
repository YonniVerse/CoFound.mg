...
test('ProjectsService.getFeed returns paginated project cards', async () => {
  ...
  assert.equal(response.items[0].id, 'proj_1')
  assert.equal(response.items[0].title, 'AgriTech Madagascar')
  assert.equal(response.items[0].status, ProjectStatus.RECRUITING)
  assert.equal(response.items[0].openPositionsCount, 1)
  assert.equal(response.items[0].membersCount, 1)
  assert.equal(response.items[0].owner?.pseudonym, 'Razafy')
