...
type FormState = { firstName: string; lastName: string; pseudonym: string; bio: string; fieldId: string; cohortYear: number | ''; skillIds: string[]; goals: string; sectorIds: string[]; availabilityHours: number | ''; visibleInTalentFeed: boolean; gender: string | null }
    ...
    void apiClient.get('/me/onboarding').then((payload) => {
      ...
      if (result.data) {
        setForm((prev) => ({
          ...
          pseudonym: typeof result.data?.pseudonym === 'string' ? result.data.pseudonym : prev.pseudonym,
