...
import { AlertCircle, ArrowLeft, Check, ChevronRight, FileText, Loader2, ShieldCheck, X } from 'lucide-react'
import { Link } from 'react-router-dom'
import { type OrganizationRequestQueueItem, organizationRequestQueueSchema } from '@cofound/shared'
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
          ...
          {selected ? <Card><CardHeader><CardTitle>{t('staff.organizations.details')}</CardTitle></CardHeader><CardContent className="space-y-6">
            ...
            {selected.supportingDocuments.length > 0 && <div className="space-y-2">{selected.supportingDocuments.map((document) => <div key={document.fileName} className="flex items-center gap-2 rounded-lg border border-border p-2 text-sm"><FileText className="h-4 w-4 text-primary" /> <span className="truncate">{document.fileName}</span><span className="ml-auto text-xs text-muted-foreground">{Math.round(document.sizeBytes / 1024)} Ko</span></div>)}</div>}
