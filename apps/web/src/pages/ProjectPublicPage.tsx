...
export default function ProjectPublicPage() {
  ...
  return <DashboardLayout><main className="mx-auto flex w-full max-w-4xl flex-col gap-6 px-6 py-8">
    ...
    {project && <>
      <header><p className="text-sm font-semibold uppercase tracking-wide text-primary">Projet</p><h1 className="mt-2 text-4xl font-bold">{project.title}</h1><p className="mt-3 text-lg text-muted-foreground">{project.pitch}</p></header>
      ...
      <section className="rounded-2xl border bg-card p-5"><h2 className="text-xl font-semibold">Publications</h2>{project.posts.length === 0 ? <p className="mt-3 text-muted-foreground">Aucune publication.</p> : <div className="mt-4 space-y-3">{project.posts.map((post) => <article key={post.id} className="rounded-xl border p-4"><p className="whitespace-pre-wrap">{post.content}</p><time className="mt-2 block text-xs text-muted-foreground" dateTime={post.createdAt.toISOString()}>{post.createdAt.toLocaleDateString("fr-FR")}</time></article>)}</div>}</section>
