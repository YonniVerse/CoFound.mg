...
import { ArrowLeft, Check, ChevronDown, Loader2, Plus, Send, X } from 'lucide-react'
import { Link, useParams } from 'react-router-dom'
import { opportunityApplicationSchema, opportunitySchema } from '@cofound/shared'
...
type Opportunity = { id: string; title: string; description: string; status: string; type: string; organizationId: string; eligibility: string | null; deadline: Date | null; seats: number | null; createdAt: Date; updatedAt: Date }
type Application = { id: string; opportunityId: string; applicantType: string; applicantId: string; message: string; status: string; rejectionReason: string | null; createdAt: Date }
...
export default function PartnerOpportunitiesPage() {
  ...
  const create = async () => {
    setBusyId('create')
    try { await apiClient.post(`/organizations/${organizationId}/opportunities`, { title, description, type: 'CALL_FOR_APPLICATIONS' }); setTitle(''); setDescription(''); setFormOpen(false); await load() } catch { setMessage(t('partner.projects.error')) } finally { setBusyId(null) }
  }
  ...
  const loadApplications = async (id: string) => {
    setBusyId(id)
    try { const result = await apiClient.get(`/organizations/${organizationId}/opportunities/${id}/applications`); setApplications((current) => ({ ...current, [id]: (result as unknown[]).map((item) => opportunityApplicationSchema.parse(item)) })) } catch { setMessage(t('partner.projects.error')) } finally { setBusyId(null) }
  }
  ...
  const decide = async (opportunityId: string, applicationId: string, status: 'ACCEPTED' | 'REJECTED') => {
  ...
  return <DashboardLayout><main className="min-h-screen bg-muted/20 p-6 lg:p-10"><div className="mx-auto max-w-6xl space-y-6">
    ...
    <header className="flex flex-wrap items-end justify-between gap-4"><div><h1 className="text-3xl font-bold tracking-tight">{t('partner.opportunities.title')}</h1><p className="mt-2 text-muted-foreground">{t('partner.opportunities.subtitle')}</p></div><Button onClick={() => setFormOpen((value) => !value)}><Plus className="mr-2 h-4 w-4" />{t('partner.opportunities.create')}</Button></header>
    {message && <p role="status" className="rounded-xl border border-primary/20 bg-primary/5 p-3 text-sm">{message}</p>}
    {formOpen && <Card><CardHeader><CardTitle>{t('partner.opportunities.create')}</CardTitle></CardHeader><CardContent className="space-y-3"><input className="h-10 w-full rounded-md border bg-background px-3" value={title} onChange={(event) => setTitle(event.target.value)} placeholder={t('partner.opportunities.titlePlaceholder')} /><Textarea value={description} onChange={(event) => setDescription(event.target.value)} placeholder={t('partner.opportunities.descriptionPlaceholder')} /><div className="flex justify-end"><Button disabled={busyId === 'create' || title.trim().length < 3 || description.trim().length < 20} onClick={() => void create()}>{t('partner.opportunities.create')}</Button></div></CardContent></Card>}
    ...
    {loading ? <div className="flex justify-center py-16"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div> : items.length === 0 ? <Card><CardContent className="p-12 text-center text-muted-foreground">{t('partner.projects.empty')}</CardContent></Card> : <div className="space-y-5">{items.map((item) => <Card key={item.id}><CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div><CardTitle>{item.title}</CardTitle><p className="mt-2 text-sm leading-6 text-muted-foreground">{item.description}</p></div><span className="rounded-full bg-muted px-3 py-1 text-xs font-semibold">{item.status === 'DRAFT' ? t('partner.opportunities.draft') : t('partner.opportunities.published')}</span></div></CardHeader><CardContent className="space-y-4"><div className="flex flex-wrap gap-2">{item.status === 'DRAFT' && <Button size="sm" disabled={busyId === item.id} onClick={() => void publish(item.id)}><Send className="mr-2 h-4 w-4" />{t('partner.opportunities.publish')}</Button>}{item.status === 'PUBLISHED' && <Button size="sm" variant="outline" disabled={busyId === item.id} onClick={() => void loadApplications(item.id)}><ChevronDown className="mr-2 h-4 w-4" />{t('partner.opportunities.application')}</Button>}</div>{applications[item.id] && <div className="divide-y rounded-xl border">{applications[item.id]!.length === 0 ? <p className="p-4 text-sm text-muted-foreground">{t('partner.projects.empty')}</p> : applications[item.id]!.map((application) => <div key={application.id} className="space-y-2 p-4"><div className="flex items-center justify-between gap-3 text-sm"><span className="font-semibold">{application.applicantType} · {application.applicantId}</span><span className="text-xs text-muted-foreground">{application.status}</span></div><p className="text-sm text-muted-foreground">{application.message}</p>{application.status === 'PENDING' && <div className="flex gap-2"><Button size="sm" onClick={() => void decide(item.id, application.id, 'ACCEPTED')} disabled={busyId === application.id}><Check className="mr-1 h-3 w-3" />{t('partner.opportunities.accept')}</Button><Button size="sm" variant="outline" onClick={() => void decide(item.id, application.id, 'REJECTED')} disabled={busyId === application.id}><X className="mr-1 h-3 w-3" />{t('partner.opportunities.reject')}</Button></div>}</div>)}</div>}</CardContent></Card>)}</div>}
