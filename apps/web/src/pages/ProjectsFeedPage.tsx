      ...
      <div className="mx-auto flex max-w-5xl flex-col gap-6 px-6 py-8 sm:px-10">
        <header className="space-y-4">
          <div>
            ...
            <h1 className="font-heading text-3xl font-bold text-foreground">{t('projects.title')}</h1>
            <p className="mt-2 text-sm text-muted-foreground">{t('projects.subtitle')}</p>
          </div>
          <div className="flex flex-col gap-3 sm:flex-row">
            <label className="relative flex-1">
              <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
              <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={t('projects.searchPlaceholder')} className="pl-9" aria-label={t('projects.searchLabel')} />
            </label>
            <div className="flex gap-2" role="group" aria-label={t('projects.filterLabel')}>
              <Button variant={selectedStatus === "RECRUITING" ? "default" : "outline"} onClick={() => setSelectedStatus("RECRUITING")}>{t('feed.projectsRecruiting')}</Button>
              <Button variant={selectedStatus === "ALL" ? "default" : "outline"} onClick={() => setSelectedStatus("ALL")}>{t('feed.allProjects')}</Button>
            </div>
          </div>
        </header>
