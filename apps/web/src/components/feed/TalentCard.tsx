  ...
  return (
    <article className="group flex min-w-0 flex-col gap-4 overflow-hidden rounded-xl border border-border bg-card p-5 shadow-2xs transition-all duration-150 hover:border-border/80 sm:p-6">
      <header className="flex min-w-0 items-start justify-between gap-4">
        ...
        <div className="flex max-w-[52%] shrink-0 items-start gap-1.5 text-right text-xs font-medium leading-snug text-muted-foreground">
      ...
      <div className="flex flex-wrap items-center gap-2 border-t border-border/50 pt-3">
        ...
        <span className="rounded-md bg-muted px-2.5 py-1 text-xs font-medium text-foreground">{categoryLabel}</span>
        <span className="mx-0.5 text-border">·</span>
        <span className="text-xs font-semibold text-muted-foreground">{t("common.skills")}:</span>
        {talent.skills.length > 0 ? talent.skills.slice(0, 4).map((skill) => (
          <span key={skill.id} className="rounded-md bg-primary/10 px-2.5 py-1 text-xs font-medium text-primary">
            {formatReferenceLabel(skill.labelKey, skill.slug)}
          </span>
        )) : <span className="text-xs text-muted-foreground">—</span>}
        {talent.skills.length > 4 && <span className="text-xs font-semibold text-muted-foreground">+{talent.skills.length - 4}</span>}
      </div>

      {talent.goals.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {talent.goals.slice(0, 2).map((goal) => (
            <span key={goal} className="rounded-md bg-muted px-2.5 py-1 text-xs font-medium text-muted-foreground">{goal}</span>
          ))}
        </div>
      )}
      ...
      <footer className="flex flex-wrap items-center justify-between gap-3 border-t border-border/50 pt-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button size="sm" className="h-9 gap-1.5 rounded-lg px-3.5 text-xs font-medium sm:text-sm">
